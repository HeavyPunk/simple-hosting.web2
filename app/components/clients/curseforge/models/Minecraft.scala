...
class GetMinecraftVersionsRequest(val sortDescending: Option[Boolean]) {
...
class GetMinecraftModloadersRequest(val version: Option[String], val includeAll: Option[Boolean]) {
  def toQueryString(): String = {
    val builder = new StringBuilder()
