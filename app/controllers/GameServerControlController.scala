...
import components.clients.controller.{
    StartGameServerRequest,
    StopGameServerRequest,
    GetServerInfoRequest,
    GetServerLogsOnPageRequest,
    SendServerMessageRequest,
}
...
class GameServerControlController @Inject() (
    ...
    def getServerInfo(): mvc.Action[AnyContent] = Action.async { implicit request =>
        if (!request.hasBody)
            Future.successful(BadRequest(serializeError("Request body is missing")))
        else {
            val rawBody = request.body.asJson
            if (!rawBody.isDefined)
                ...
                val reqObj = jsonizer.deserialize(rawBody.get.toString, classOf[components.clients.controller.GetServerInfoRequest])
                ...
                if (gameServer.isEmpty)
                    ...
                    if (user.isEmpty)
                        ...
                        val serverResp = controllerClient.servers.getServerInfo(
                            new io.github.heavypunk.controller.client.contracts.server.GetServerInfoRequest(reqObj.postSystem),
                            Duration.ofMinutes(2)
                        )
                        if (!serverResp.success) Future.successful(InternalServerError(jsonizer.serialize(serverResp)))
                        else {
                            gameServer.get.isActiveServer = false
                            gameServerStorage.update(gameServer.get)
                            Future.successful(Ok(jsonizer.serialize(serverResp)))
                        }
                    }
                }
            }
    ...
    def sendServerMessage() = Action.async {implicit request =>
        if (!request.hasBody)
            ...
            if (!rawBody.isDefined)
                ...
                val reqObj = jsonizer.deserialize(rawBody.get.toString, classOf[SendServerMessageRequest])
                ...
                if (gameServer.isEmpty)
                    ...
                    if (user.isEmpty)
                        ...
                        val serverResp = controllerClient.servers.sendMessage(new SendMessageRequest(reqObj.message, reqObj.postSystem), Duration.ofMinutes(2))
    ...
    def getServerLogs() = Action.async { implicit request => 
        if (!request.hasBody)
            ...
            if (!rawBody.isDefined)
                ...
                val reqObj = jsonizer.deserialize(rawBody.get.toString, classOf[GetServerLogsOnPageRequest])
                ...
                if (gameServer.isEmpty)
                    ...
                    if (user.isEmpty)
                        ...
                        var serverResp: GetServerLogsResponse = 
                            if (reqObj.isLastLogs) controllerClient.servers.getServerLogsLastPage(Duration.ofMinutes(2))
                            else controllerClient.servers.getServerLogs(new GetServerLogsRequest(reqObj.page), Duration.ofMinutes(2))
